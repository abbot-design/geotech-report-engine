...
// not print a stray separator where the value would have gone. It also loses
// a key now and then (jobNo was the first); a report that still carries one
// has to import and render with the orphaned value simply ignored.
//
  ...
  test('old report saved before waterDepth existed imports and renders cleanly', async ({ page }) => {
    ...
    const oldReport = await page.evaluate(() => {
      ...
      return {
        ...
        d: {
          jobNo: 'OLD-001', // removed from the engine 2026-09-17; must be ignored, not fatal
          version: '1', dateIssued: now.toISOString().slice(0, 10),
    ...
    expect(html, 'an orphaned jobNo is ignored, not rendered').not.toContain('OLD-001');
  });
});
